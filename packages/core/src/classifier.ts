...
export class PageClassifier {
     ...
     */
    async classify(url: string, page: Page, response: Response | null = null): Promise<ClassificationResult> {
        ...
        try {
            const types = await page.evaluate(() => {
                ...
                scripts.forEach(s => {
                    try {
                        ...
                        Array.isArray(json) ? json.forEach(check) : check(json);
                    } catch (e) { }
                });
                return found;
            });
            ...
            if (response && jsonLdTypes.size > 0 && response.ok()) {
                ...
                for (const t of Array.from(jsonLdTypes)) {
                    ...
                    if (!raw.includes(`"@type": "${t}"`) && !raw.includes(`"@type":"${t}"`)) {
                        // Could be dynamic
        ...
        for (const [type, def] of Object.entries(definitions)) {
            ...
            if (def.excludePattern) {
                try {
                    ...
                    console.warn(`Invalid excludePattern regex for type '${type}': ${def.excludePattern}`);
                }
            }
            ...
            if (def.schemaType) {
                const required = Array.isArray(def.schemaType) ? def.schemaType : [def.schemaType];
                // Check if ANY required type is present in jsonLdTypes, case-insensitive check
                const hasMatch = required.some((t: string) =>
                    jsonLdTypes.has(t) ||
                    Array.from(jsonLdTypes).some(found => found.toLowerCase() === t.toLowerCase())
                );
            ...
            if (!isMatch && def.selector) {
                try {
                    ...
                    console.warn(`Error checking selector '${def.selector}' for type '${type}':`, e);
                }
            }
            ...
            if (!isMatch && def.urlPattern) {
                try {
                    ...
                    console.warn(`Invalid urlPattern regex for type '${type}': ${def.urlPattern}`);
                }
            }
        ...
        return {
            ...
            meta: {
                jsonLdSource: isDynamic ? 'dynamic' : (jsonLdTypes.size > 0 ? 'static' : 'none'),
                isDynamic
            }
        };
    }
